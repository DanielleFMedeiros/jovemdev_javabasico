...
public class Util {
		...
		while (cadastrarAutor) {
			...
			if (sexo == null || sexo.trim().isEmpty() || (sexo.charAt(0) != 'F' && sexo.charAt(0) != 'M')) {
				JOptionPane.showMessageDialog(null,
						"Sexo inválido. Por favor, digite 'F' para feminino ou 'M' para masculino.");
				continue;
			}
	...
	public static void listarLivrosPorSexoDosAutores(ArrayList<Livro> livros) {
		char sexo = JOptionPane
				.showInputDialog("Informe o sexo dos autores que deseja pesquisar (M para masculino, F para feminino):")
				.charAt(0);
		...
		if (livrosPorSexo.isEmpty()) {
			JOptionPane.showMessageDialog(null,
					"Nenhum livro cadastrado escrito apenas por " + (sexo == 'M' ? "homens." : "mulheres."));
			return;
		}

		JOptionPane.showMessageDialog(null,
				"=== Livros escritos apenas por " + (sexo == 'M' ? "homens ===" : "mulheres ==="));
